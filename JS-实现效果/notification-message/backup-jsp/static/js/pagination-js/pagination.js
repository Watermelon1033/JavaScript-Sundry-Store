...
PAG.prototype = {
    ...
    initialize: function (parametersObj) {
        ...
        if (store.totalPages <= store.startShowMaxBtn) {
            ...
            // 如果store.totalPages = 1 "下一页"按钮也要添加 disabled
            if (store.totalPages === 1) {
                var lastLi = store.container.getElementsByTagName("li")[2];
                if (Tools.hasClass(lastLi, "disabled") === false) {
                    Tools.addClass(lastLi, "disabled");
                }
            }
    ...
    renderPureNumPag: function (pages, store) {
        ...
        var paginationStr = '<ul class="pagination" id="pagination">';
    ...
    switchPag: function () {
        ...
        EventUtil.addHandler(store.container, "click", function (event) {
            ...
            if (target.hasAttribute("data-num")) {
                ...
                // "上一页/下一页"按钮
                var previousBtn = Tools.selectorEle(store.previousBtn, false);
                var nextBtn = Tools.selectorEle(store.nextBtn, false);
                ...
                var curLiDataNum = parseInt(target.getAttribute("data-num"), 10);
                ...
                if (store.totalPages <= store.startShowMaxBtn) {
                    ...
                    console.log("当前点击的页码: ", target.getAttribute("data-num"));
                    var currentPageList = AjaxConfMap.fnCurrentPageList(parseInt(target.getAttribute("data-num"), 10));
                    ...
                    var targetDataNum = parseInt(target.getAttribute("data-num"), 10);
                    var allLis = [].slice.call(that.querySelectorAll(".num-pag"));

                    console.log("单独点击数字键时当前页码: ", targetDataNum);
                    ...
                    allLis.forEach(function (item) {
                        ...
                        var itemDataNum = parseInt(item.getAttribute("data-num"), 10);
                        if (targetDataNum === itemDataNum) {
                            Tools.addClass(item, "pag-active");

                            // 调用 fnCurrentPagList 获取当前页面需要展示的列表集合
                            var currentPageList = AjaxConfMap.fnCurrentPageList(parseInt(item.getAttribute("data-num")));
                            NotiObjConfMap.createInfoList(currentPageList);
                        }
            ...
            // 复用 if 语句
            var fnReusingIfStatements = function (recordPagNum, that) {
                ...
                console.log("复用语句执行后当前页码: ", recordPagNum);

                // 调用 fnCurrentPagList 获取当前页面需要展示的列表集合
                var currentPageList = AjaxConfMap.fnCurrentPageList(recordPagNum);
                NotiObjConfMap.createInfoList(currentPageList);
            ...
            if (Tools.hasClass(target, "previous-btn")) {
                ...
                // 找到当前 container 下 class 含有 pag-active 的 li
                ...
                var activePreEle = activeEle.previousSibling;
                var preEleDataNum = parseInt(activePreEle.getAttribute("data-num"), 10);

                // 调用 fnCurrentPagList 获取当前页面需要展示的列表集合
                NotiObjConfMap.createInfoList(AjaxConfMap.fnCurrentPageList(preEleDataNum));


                // 总页数 <= 开头最大显示页码(默认：8) 一次创建完所有分页。
                if (store.totalPages <= store.startShowMaxBtn) {

                    // 首先移除所有数字 li 上的 pag-active
                    ...
                    if (parseInt(activeEle.getAttribute("data-num"), 10) === store.totalPages) {
                        console.log(activeEle.nextSibling);
            ...
            if (Tools.hasClass(target, "next-btn")) {
                ...
                var activeNextEle = activeEle.nextSibling;
                var nextEleDataNum = parseInt(activeNextEle.getAttribute("data-num"), 10);

                // 调用 fnCurrentPagList 获取当前页面需要展示的列表集合
                NotiObjConfMap.createInfoList(AjaxConfMap.fnCurrentPageList(nextEleDataNum));
