...
var addClass = function (cla, ele) {
    if (this.hasClass(cla, ele)) {
        return null;
    }
    ele.className = (ele.className || "") + " " + cla;
};
var removeClass = function (cla, ele) {
    var names = ele.className.split(/\s+/);
    var i = 0,
        len = names.length,
        arr = [];
...
var selectorEle = function (selector, all) {
    return all ? document.querySelectorAll(selector) : document.querySelector(selector);
};
...
// 截取小数点后两位.  \(?:): 非捕获型分组。 ?: 匹配前面元字符0次或1次，/ba?/: 将匹配b 或 ba
