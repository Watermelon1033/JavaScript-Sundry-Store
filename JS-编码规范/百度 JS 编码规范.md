...
##### [强制] `switches` 下的 `case` 和 `default` 必须增加一个缩进层级。
...
##### [强制] `if / else / for / while / function / switches / do / try / catch / finally` 关键字后，必须有一个空格。
...
##### [强制] 在函数调用、函数声明、括号表达式、属性访问、`if / for / while / switches / catch` 等语句中，`()` 和 `[]` 内紧贴括号部分不允许有空格。
...
##### [建议] 对于相同变量或表达式的多值条件，用 `switches` 代替 `if`。
